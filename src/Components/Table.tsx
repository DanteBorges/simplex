  ...
  return (
    <div>
      <div className={styles.divText}>
        ...
        {nicePoint != undefined &&
          nicePoint.map((item: any) => (
            <label>
              <strong>
                {" "}
                X1:{item.solution.X1} X2:{item.solution.X2} X3:
                {item.solution.X3}{" "}
              </strong>
            </label>
          ))}
      ...
      <div className={styles.goodPoint}>
        {Array.isArray(receivedData) &&
          receivedData.map(
            (item: any) =>
              item.erro !== "" && (
                <strong>Não foi possível encontrar a solução inteira</strong>
              )
          )}
      </div>

      {Array.isArray(receivedData) &&
        receivedData.map((rd: any) => rd.erro !== "" && <p>{rd.erro}</p>)}
                ...
                <tbody>
                  ...
                  {rd.base.map((item: string, index: number) => (
                    <tr>
                      <td>{item}</td>
                      {rd.table[index].map(
                        (subItem: any, indexNumber: number) =>
                          index == rd.pivo[0] && indexNumber == rd.pivo[1] ? (
                            <td
                              style={{
